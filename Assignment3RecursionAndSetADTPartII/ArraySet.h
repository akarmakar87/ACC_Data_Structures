...
public:
	ArraySet();
  ArraySet(const ItemType anArray[], const int numElements);
...
template<class ItemType>
ArraySet<ItemType>::ArraySet(const ItemType anArray[], const int numElements) {
...
{
	bool canAdd = (itemCount < maxItems) && !contains(newEntry);
	if (canAdd)
	{
